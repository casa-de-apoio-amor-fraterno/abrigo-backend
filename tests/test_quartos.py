...
def test_listar_nao_traz_inativo_por_padrao(client, db_session):
    db_session.add_all(
        [
            Quarto(numero="11", leito="04", ativo=True),
            Quarto(numero="99", leito="00", ativo=False),
        ]
    )
    db_session.commit()
...
def test_criar_e_buscar_quarto(client):
    resposta = client.post(
        "/api/quartos",
        json={"descricao": "Cadeirante", "numero": "16", "leito": "2 leitos"},
    )
    ...
    assert corpo["leito"] == "2 leitos"
    assert corpo["ativo"] is True


def test_atualizar_quarto(client):
    resposta = client.post("/api/quartos", json={"numero": "20", "leito": "02"})
    quarto_id = resposta.json()["id"]

    resposta = client.put(
        f"/api/quartos/{quarto_id}",
        json={"descricao": "Reformado", "numero": "20", "leito": "03"},
    )

    assert resposta.status_code == 200
    assert resposta.json()["leito"] == "03"
    assert resposta.json()["descricao"] == "Reformado"


def test_inativar_quarto(client):
    resposta = client.post("/api/quartos", json={"numero": "21", "leito": "02"})
    quarto_id = resposta.json()["id"]
